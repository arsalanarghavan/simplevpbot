...
class SimpleVPBot_UI_Reply_Router {
	...
	public static function try_dispatch_hub_action( array $ctx ) {
		...
		if ( isset( $route['hub'] ) ) {
			SimpleVPBot_Handler_Admin_Hub::send_submenu( $plat, $chat_id, (string) $route['hub'], $ctx );
			return true;
		}
		...
		if ( isset( $route['hub_dispatch_cb'] ) ) {
			SimpleVPBot_Handler_Admin_Hub::dispatch_reply_as_callback( $ctx, (string) $route['hub_dispatch_cb'] );
			return true;
		}
		...
		if ( isset( $route['bulk_days'] ) ) {
			SimpleVPBot_Handler_Admin_Hub::router_bulk_days_confirm( $plat, $chat_id, (int) $route['bulk_days'] );
			return true;
		}
		if ( isset( $route['bulk_gb'] ) ) {
			SimpleVPBot_Handler_Admin_Hub::router_bulk_gb_confirm( $plat, $chat_id, (int) $route['bulk_gb'] );
			return true;
		}
		return false;
	}
