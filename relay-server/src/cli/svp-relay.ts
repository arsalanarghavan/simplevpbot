...
function cmdTenantShow(id: string): void {
...
function cmdDomainsList(): void {
  console.log(collectAllDomains().join("\n"))
}

function cmdDomainAdd(domain: string): void {
...
function cmdDomainRemove(domain: string): void {
  const tenantId = flag("--tenant") || listTenants()[0]?.tenant_id
...
function cmdNginxRender(): void {
  const out = renderNginxConfig({ outPath: flag("--out") })
  console.log(`wrote ${out}`)
}

function cmdSslIssue(domain: string): void {
...
switch (cmd) {
  ...
  case "status":
    ...
    break
  case "tenants":
    if (sub === "list") cmdTenantsList()
    else help()
    break
  case "tenant":
    if (sub === "show" && argv[2]) cmdTenantShow(argv[2])
    else help()
    break
  case "domains":
    if (sub === "list") cmdDomainsList()
    else help()
    break
  case "domain":
    if (sub === "add" && argv[2]) cmdDomainAdd(argv[2])
    else if (sub === "remove" && argv[2]) cmdDomainRemove(argv[2])
    else help()
    break
  case "nginx":
    if (sub === "render") cmdNginxRender()
    else help()
    break
  case "ssl":
    if (sub === "issue" && argv[2]) cmdSslIssue(argv[2])
    else if (sub === "renew") renewSsl((flag("--method") || "certbot") as "certbot" | "acme")
    else help()
    break
  case "config":
    if (sub === "migrate") {
