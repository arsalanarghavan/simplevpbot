...
async function menuDashboard(): Promise<void> {
...
async function menuService(): Promise<void> {
  ...
  try {
    if (action === "status") serviceStatus()
    else if (action === "start") serviceStart()
    else if (action === "stop") serviceStop()
    else if (action === "restart") serviceRestart()
  } catch (e) {
...
async function menuTenants(): Promise<void> {
  const action = await select({
    message: "Tenants",
    choices: [
...
async function menuDomains(): Promise<void> {
  const action = await select({
    message: "Domains",
    choices: [
      { name: "List", value: "list" },
...
async function menuSsl(): Promise<void> {
  const action = await select({
    message: "SSL",
    choices: [
  ...
  if (action === "renew") {
    const method = await select({
      ...
      choices: [
        { name: "certbot", value: "certbot" as const },
        { name: "acme.sh", value: "acme" as const },
      ],
    })
    try {
      renewSsl(method)
    ...
    try {
      if (method === "acme") issueSslAcme(domain, email)
      else issueSslCertbot(domain, email)
      try {
...
async function menuNginx(): Promise<void> {
  const action = await select({
    message: "Nginx",
    choices: [
  ...
  if (action === "path") {
    ...
    if (enable) {
      try {
        execSync(
          `ln -sf ${env.nginxConfigPath} /etc/nginx/sites-enabled/svp-relay.conf`,
...
async function menuWpSetup(): Promise<void> {
...
async function menuLogs(): Promise<void> {
...
async function menuInstall(runInstall: (args: string[]) => void): Promise<void> {
...
export async function runPanel(runInstall: (args: string[]) => void): Promise<void> {
  ...
  for (;;) {
    ...
    if (choice === "dashboard") await menuDashboard()
    else if (choice === "service") await menuService()
    else if (choice === "tenants") await menuTenants()
    else if (choice === "domains") await menuDomains()
    else if (choice === "ssl") await menuSsl()
    else if (choice === "nginx") await menuNginx()
    else if (choice === "wp") await menuWpSetup()
    else if (choice === "logs") await menuLogs()
    else if (choice === "doctor") await menuDoctor()
    else if (choice === "install") await menuInstall(runInstall)
  }
}
